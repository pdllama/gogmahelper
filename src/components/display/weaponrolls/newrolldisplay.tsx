...
export default function NewRollDisplay({current_weapons, rollType}:RollDisplayProps) {
    ...
    return (
        ...
        <RollDisplayWrapper no_shadow classes="border-dashed border-cyan-500 border relative overflow-hidden">
            <>
            <Button classes="grid-min-height grid-max-width flex justify-center items-center rounded-xl border-none hover:bg-cyan-950 z-1" disableRipple onClick={activate_form}>
                <img src='icons/app/plus.png' width='80px' height='80px'/>
            </Button> 
            <div className='size-full flex flex-col items-center rounded-xl absolute top-[-100%] bg-cyan-800 z-3 slide-form gap-2' ref={form_ref}>
                ...
                <div className='size-full flex items-end justify-center p-1 gap-2'>
                    <Button 
                        ...
                        onClick={(!form.weapon || !form.element) ? undefined : 
                            () => {
                                add_new_weapon(update_stats, add_alert, form.weapon as weapons, form.element as elements, roll_type.SKILLS)
                                const divEle = form_ref.current as HTMLDivElement;
                                divEle.style.top = '-100%';
